/*
 * demo_plain_io.c -- sync-looking code, run asynchronously.
 *
 * Four reads are submitted and nothing waits for them. The buffers are then handed
 * to count_words(), which is ordinary C, has no marker and no async call, and
 * cannot tell that they were still being filled.
 *
 * The counter worth watching is the per-file one: while a batch is outstanding
 * every byte access enters the resolve slow path, and the range memo answers all
 * but a couple of them without touching the request table. docs/ARCHITECTURE.md
 * sections 7 and 8 have the story behind that.
 *
 * These are cache-resident files on purpose, which is the regime where overlapping
 * buys nothing. For the one where it pays, see demo_wordcount.c.
 *
...
int main(void) {
  ...
  /* Bookkeeping only: the counts above are already computed. */
  long total = 0;
  for (int i = 0; i < FILES; i++)
